#!/bin/bash
# setup-kubectl-tunnel.sh
# Sets up SSH tunnel for kubectl access to k3s cluster
#
# Usage: ./scripts/setup-kubectl-tunnel.sh
...
if [ -f "$TUNNEL_PID_FILE" ]; then
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        echo "To restart, first run: kill $PID"
        exit 0
    else
        # Stale PID file
...
if [ ! -f "$KUBECONFIG_PATH" ]; then
    ...
    sed -i.bak 's/127.0.0.1:6443/127.0.0.1:6443/g' "$KUBECONFIG_PATH"
    sed -i.bak 's/server: https:\/\/192.168.122.10:6443/server: https:\/\/127.0.0.1:6443/g' "$KUBECONFIG_PATH"
...
ssh -f -N -L 6443:192.168.122.10:6443 kpi-cp-01
